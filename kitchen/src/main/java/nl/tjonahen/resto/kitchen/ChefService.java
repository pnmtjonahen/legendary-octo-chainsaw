...
public class ChefService {
  ...
  private static final List<Dish> DISHES =
      Arrays.asList(
          ...
          Dish.builder()
              ...
              .preparationTime(120L)
              .name("Scrambled eggs")
              .description("Scra,mbled eggs, roasted red pepper and garlic, with green onions")
              .price(750L)
              .build(),
          Dish.builder()
              .ref("5")
  ...
  public void processCoupon(CouponMessage dishes) {
    ...
    for (Coupon dish : dishes.getItems()) {
      DISHES.stream()
          .filter(d -> d.getRef().equals(dish.getRef()))
          .mapToLong(Dish::getPreparationTime)
          .forEach(
              d -> {
                try {
                ...
                log.info(
                    "Dish {} is ready for service for order {}", dish.getId(), dishes.getOrderid());
                restTemplate.postForLocation(
                    String.format(
                        "%s/api/order/%d/serve/%d", dinerUrl, dishes.getOrderid(), dish.getId()),
                    Void.class);
              });
    }
  }
