...
public class OrderController {
  private static final String ERROR_WHILE_UPDATEING_ORDERSTATUS_ERROR_MESSAGE =
      "Error while updateing orderstatus %s";
  ...
  public OrderController(
      ...
      final OrderStatusBroker orderStatusBroker) {
      ...
      final var billItems =
          order.get().getOrderItems().stream()
              .map(
                  item ->
                      BillItem.builder()
                          .name(orderService.getName(item))
                          .quantity(item.getQuantity())
                          .price(orderService.getPrice(item))
                          .build())
              .toList();
      return ResponseEntity.ok(
          Bill.builder()
              .items(billItems)
              .total(billItems.stream().map(BillItem::getTotal).reduce(0L, Long::sum))
              .build());
    }
  ...
  public void throwInternalServerError(IOException ex) throws HttpServerErrorException {
    final var message =
        String.format(ERROR_WHILE_UPDATEING_ORDERSTATUS_ERROR_MESSAGE, ex.getMessage());
    log.error(message);
            ...
            order -> {
              try {
                order.getOrderItems().stream()
                    .filter(
                        item ->
                            item.getOrderItemType() == OrderItemType.DISH
                                && item.getId().equals(dishid))
                    .forEach(item -> item.setPrepared(true));
                if (order.getOrderItems().stream()
                    .noneMatch(
                        item ->
                            item.getOrderItemType() == OrderItemType.DISH && !item.isPrepared())) {
  ...
  @PostMapping
  @Transactional
  public ResponseEntity<ResponseOrder> placeOrder(
      @RequestBody final List<RequestedItem> orderItems, final UriComponentsBuilder builder) {
                        ...
                        .map(
                            item ->
                                OrderItem.builder()
                                    ...
                                    .quantity(item.getQuantity())
                                    .orderItemType(
                                        item.getType() == RequestedItemType.DISH
                                            ? OrderItemType.DISH
                                            : OrderItemType.DRINK)
                                    .build())
                        .toList())
    ...
    orderService.processDishes(
        order.getId(),
        order.getOrderItems().stream()
            .filter(item -> item.getOrderItemType() == OrderItemType.DISH)
            .toList());

    orderService.processDrinks(
        order.getId(),
        order.getOrderItems().stream()
    ...
    return new ResponseEntity<>(
        ResponseOrder.builder()
            .ref(order.getId())
            .billUrl(uriComponents.toUri().toString())
            .build(),
        HttpStatus.ACCEPTED);
  }
}
