...
/**
 * OrderStatusBroker is a WebSocket that allows the server to push an order status update to the
 * client.
 */
@Slf4j
@Service
public class OrderStatusWebSocketHandler implements WebSocketHandler {
  ...
  /**
   * Handle he websocket message, The frontend wil send the orderid to the websocket to indicate on
   * what order it is waiting.
   *
   * @param session -
   ...
   */
  @Override
  public void handleMessage(WebSocketSession session, WebSocketMessage<?> encodedMessage)
      throws Exception {
    final var value =
        new ObjectMapper().readValue(encodedMessage.getPayload().toString(), OrderStatus.class);
