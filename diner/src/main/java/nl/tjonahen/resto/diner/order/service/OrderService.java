...
public class OrderService {
  ...
  public void processDrinks(Long orderid, List<OrderItem> drinks) {
    ...
    rabbitTemplate.convertAndSend(
        DinerApplication.BARTENDER_EXCHANGE,
        DinerApplication.BARTENDER_KEY,
  ...
  public void processDishes(Long orderid, List<OrderItem> dishes) {
    ...
    dishes.stream()
        .forEach(
            item -> {
              HttpEntity<RequestedMessage> request =
                  new HttpEntity<>(
                      RequestedMessage.builder()
                          .items(
                              Arrays.asList(
              ...
              restTemplate.exchange(
                  chefUrl + "/api/order", HttpMethod.POST, request, RequestedMessage.class);
            });
  }
  ...
  public Long getPrice(OrderItem item) {
    if (item.getOrderItemType() == OrderItemType.DISH) {
      final var dish =
          restTemplate.getForObject(chefUrl + "/api/dish/" + item.getRef(), Dish.class);
      return dish == null ? 0L : dish.getPrice();
    }
    final var drink =
        restTemplate.getForObject(bartenderUrl + "/api/drink/" + item.getRef(), Drink.class);
